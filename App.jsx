...
export default function PriceScanner() {
  const [products, setProducts] = useState({}); // barcode -> {name, price}
  ...
  useEffect(() => {
    ...
    // Seed one test article the first time, so there's a barcode to try.
    if (Object.keys(loadedProducts).length === 0) {
      loadedProducts = {
        "4104420000015": { name: "Hacker Pschorr 0.5L", price: 185.0 },
      };
      try {
  ...
  const openProduct = (barcode) => {
    const existing = products[barcode];
    setScannedBarcode(barcode);
    setEditName(existing ? existing.name : "");
    setEditPrice(existing ? String(existing.price) : "");
    setMode("editing");
  };
  ...
  const saveAndQueue = async () => {
    ...
    const nextProducts = {
      ...products,
      [scannedBarcode]: { name: editName.trim(), price: priceNum },
    };
      ...
      {mode === "editing" && (
        ...
        >
          <div style={{ fontFamily: FONT_MONO, fontSize: 12, color: MUTE, marginBottom: 12 }}>
            {scannedBarcode}
            {products[scannedBarcode] ? "" : "  ·  new product"}
          </div>
